...
Generates PCAP traffic and feature vectors for each stage from:
"Threat Scenario Generation for IEC104 Cyber Defense" (Table III).
...
Usage:
    ...
    python simulate_8_stages.py --stage 7 --output-dir output/sim_8stages
    python simulate_8_stages.py --features-only  # Skip PCAP, generate feature dicts
"""
...
def build_iframe(
    ...
    asdu_header = struct.pack("<BBB",
                              ...
                              vsq,
                              cot & 0x3F)
    asdu_header += struct.pack("<B", originator)  # Originator address
    asdu_header += struct.pack("<H", common_addr)  # Common address
...
def build_setpoint_normalized(
    ...
    # NVA (2 bytes) + QOS (1 byte, 0 = default)
...
def _pcap_packet(ts: float, payload: bytes) -> bytes:
    ...
    ip_header = struct.pack(">BBHHHBBH4s4s",
                            ...
                            b"\xc0\xa8\x01\x0A",   # 192.168.1.10 (attacker)
                            b"\xc0\xa8\x01\x1C")   # 192.168.1.28 (RTU)

    # TCP header (20 bytes, no options)
    tcp_header = struct.pack(">HHIIBBHHH",
                             12345, IEC104_PORT,  # src/dst port
                             1000, 2000,          # seq/ack
...
def _base_ts() -> float:
    """Base timestamp for simulation (2026-02-17T10:00:00Z)."""
...
def stage_1_discovery(base_ts: float | None = None) -> StageResult:
    ...
    for addr in [1, 3, 5]:  # Only 3 stations respond
        for ioa in range(1, 6):
            # M_SP_NA_1 response: single-point with quality byte
    ...
    result = StageResult(
        ...
        mitre_id="T0888",
        packets=packets,
        event_count=len(station_addresses) + 15,  # interrogations + responses
        description="Interrogation sweep across 20 station addresses, 3 responded",
    )
    result.features = _stage_to_features(result, {
        "n_events": result.event_count,
        "n_flows": 1,
        "total_bytes": len(packets) * 40,
        "unique_typeIDs": 2,  # C_IC_NA_1 + M_SP_NA_1
        "unique_cots": 3,     # ACT + INROGEN + SPONT
        "unique_ioas": 5,
        ...
        "dominant_ioa_ratio": 0.14,  # IOA 0 (interrogation) is most common
        "ioa_entropy": 1.6,
        "mean_asdu_length": 14,
        "typeID_entropy": 0.95,
        "cot_entropy": 1.2,
...
def stage_2_collection_monitor(base_ts: float | None = None) -> StageResult:
    ...
    ts = (base_ts or _base_ts()) + 10  # 10s after discovery
    seq = SequenceCounter()
    packets = []

    # Read specific IOAs frequently (every 2 seconds for 60 seconds)
    target_ioas = [100, 200, 300, 400, 500]  # Known high-value IOAs
    common_addr = 1

    for cycle in range(30):  # 30 cycles = 60 seconds
        for ioa in target_ioas:
            packets.append((ts, build_read_command(common_addr, ioa, seq)))
            ts += 0.05

            # RTU responds with measurement
            rtu_seq = SequenceCounter(send_seq=cycle * 5)
            nva = struct.pack("<hB", int(50 * 327.67), 0)  # ~50% normalized
            packets.append((ts, build_monitor_response(
                TYPEID_M_ME_NA_1, common_addr, ioa, nva, rtu_seq, cot=COT_REQUEST
            )))
            ts += 0.05
        ts += 1.9  # Wait to fill 2-second cycle
...
def stage_3_collection_automated(base_ts: float | None = None) -> StageResult:
    ...
    common_addr = 1
    poll_ioas = list(range(1, 51))  # Poll 50 IOAs continuously
...
def stage_4_command_control(base_ts: float | None = None) -> StageResult:
    ...
    # Establish a second session (STARTDT)
    packets.append((ts, build_uframe(UFRAME_STARTDT_ACT)))
    ts += 0.05
    packets.append((ts, build_uframe(UFRAME_STARTDT_CON)))
    ts += 0.1

    # Keepalive to maintain presence
    for i in range(10):
        packets.append((ts, build_uframe(UFRAME_TESTFR_ACT)))
        ts += 0.05
        packets.append((ts, build_uframe(UFRAME_TESTFR_CON)))
        ts += 5.0  # 5s keepalive interval

    # Relay a few commands through the C2 channel
    common_addr = 1
    for ioa in [100, 200]:
    ...
    result.features = _stage_to_features(result, {
        ...
        "command_vs_monitor_ratio": 999.0,  # All commands, no monitors
...
def stage_5_brute_force_io(base_ts: float | None = None) -> StageResult:
    ...
    common_addr = 1
    ioa_range = list(range(100, 2001))  # Sweep IOAs 100-2000

    for ioa in ioa_range:
        # Send single command (ON)
        packets.append((ts, build_single_command(common_addr, ioa, 0x01, seq)))
        ts += 0.03  # ~33 commands/second
    ...
    result.features = _stage_to_features(result, {
        ...
        "ioa_entropy": math.log2(len(ioa_range)),  # ~10.9 bits (near-uniform)
        "mean_asdu_length": 15,
        "typeID_entropy": 0.0,
        "cot_entropy": 0.0,
        "iat_mean": 0.03,
...
def stage_6_modify_parameter(base_ts: float | None = None) -> StageResult:
    ...
    common_addr = 1
    # Target safety-critical setpoints
    targets = [
    ...
    for ioa, value in targets:
        ...
        ts += 2.0  # 2s between parameter changes

        # Fake activation confirm from RTU
...
def stage_7_unauthorized_command(base_ts: float | None = None) -> StageResult:
    ...
    # Send commands with wrong/missing COT sequences (no prior interrogation,
    # no activation->confirm handshake, just raw commands)
    attack_commands = [
        # (ioa, typeID, cot, description)
    ...
    result.features = _stage_to_features(result, {
        ...
        "unique_typeIDs": 2,
        "unique_cots": 5,  # SPONT, ACT, INROGEN, UNKNOWN_COT, DEACT
        "unique_ioas": 8,
        "unique_common_addresses": 1,
        "interrogation_count": 0,
        ...
        "unexpected_cot_sequence": 6,  # SPONT, INROGEN, UNKNOWN_COT, DEACT without ACT
...
def stage_8_manipulation_of_control(base_ts: float | None = None) -> StageResult:
    ...
    common_addr = 1
    # Target a critical breaker/valve -- rapid ON/OFF cycling to cause physical damage
    target_ioas = [500, 501, 502]  # Multiple critical outputs
    ...
    result = StageResult(
        ...
        mitre_id="T0831",
        packets=packets,
        event_count=20 * 3 * 2,  # 120 commands
...
def _stage_to_features(result: StageResult, overrides: dict) -> dict:
    """Build a feature dict from stage overrides.

    Returns only the explicitly specified features (no external dependencies).
    """
    features = {}
...
def run_all_stages(output_dir: Path | None = None) -> list[StageResult]:
    ...
    if output_dir:
        ...
        with open(output_dir / "stage_features.json", "w", encoding="utf-8") as f:
            json.dump(feature_dicts, f, indent=2, default=str)

        logger.info("Wrote PCAPs and features to %s", output_dir)

    return results


def main():
    parser = argparse.ArgumentParser(description="8-Stage IEC-104 Attack Scenario Simulation")
    parser.add_argument("--output-dir", type=Path,
                        default=Path("output") / "sim_8stages",
                        help="Output directory for PCAPs and feature JSONs")
    ...
    with open(args.output_dir / "stage_features.json", "w", encoding="utf-8") as f:
        json.dump(feature_dicts, f, indent=2, default=str)
